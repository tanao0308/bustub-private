...
void SortExecutor::Init() {
  ...
  std::sort(tuples.begin(), tuples.end(), [this](const Tuple &left, const Tuple &right) {
...
auto SortExecutor::Cmp(const Tuple& left_tuple, const Tuple& right_tuple) const -> bool {
  for(auto cond : plan_->GetOrderBy()) {
    ...
    if(((order_by == OrderByType::ASC || order_by == OrderByType::DEFAULT) && left_value.CompareLessThan(right_value) == CmpBool::CmpTrue) ||
